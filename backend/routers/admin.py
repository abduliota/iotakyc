...
def _list_from_supabase(status, page, limit) -> dict:
    ...
    # ── Step 1: fetch sessions (no join — PostgREST join requires explicit FK hint) ──
    sessions_query = db.table("kyc_sessions").select("*").neq("status", "in_progress")
    if status:
        sessions_query = sessions_query.eq("status", status)

    count_query = db.table("kyc_sessions").select("id", count="exact").neq("status", "in_progress")
    if status:
    ...
    offset = (page - 1) * limit
    sessions_res = sessions_query.order("updated_at", desc=True).range(offset, offset + limit - 1).execute()
    sessions = sessions_res.data or []

    if not sessions:
        return {"items": [], "total": total, "page": page}

    # ── Step 2: fetch all kyc_data for these session IDs in one query ──
    session_ids = [s["id"] for s in sessions]
    kyc_res = db.table("kyc_data").select("*").in_("session_id", session_ids).execute()
    kyc_map = {row["session_id"]: row for row in (kyc_res.data or [])}

    # ── Step 3: fetch all audit logs for these sessions in one query ──
    audit_res = db.table("audit_logs").select("*").in_("session_id", session_ids).order("created_at").execute()
    audit_map: dict[str, list] = {}
    for log in (audit_res.data or []):
        sid = log["session_id"]
        audit_map.setdefault(sid, []).append(log)
    ...
    for session in sessions:
        sid = session["id"]
        kyc = kyc_map.get(sid, {})
        audit = audit_map.get(sid, [])

        items.append({
            **session,
            # iqama may be in kyc_data even if not on session row
            "iqama":           kyc.get("iqama") or session.get("iqama") or "—",
...
def update_submission(session_id: str, body: ActionRequest):
    valid_actions = ["approved", "rejected", "flagged", "requested_info", "under_review"]
